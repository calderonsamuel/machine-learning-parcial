...
```{r}
suscripcion_processed <- suscripcion |> 
  mutate_if(is.character, as.factor)
```

```{r}
suscripcion_processed$`Subscripcion Deposito` 
```
...
data_split <- initial_split(suscripcion_processed, prop = 3/4)
...
```{r}
my_recipe <- recipe(`Subscripcion Deposito` ~ ., data = train_data) |> 
